 ...
 *
 * Two GETs and nothing else. There is no write path here and there must never be
 * one: the trail is written by `modules/audit/audit.service.ts` and by nothing
 * else, and an endpoint that could edit or delete an entry would make the whole
 * table unusable as evidence. No POST, no PATCH, no DELETE.
 *
