...
/*
 * `npm run db:reset` — empty the database of customer data, keeping the
 * bootstrap admin and everything the admin has configured.
 *
 ...
 *
 *   Operational data — orders, quotes, payments, mail, conversations, customers.
 *   Always cleared. This is what "make it fresh" means.
 *
...
// Opt in to clearing the admin's own configuration as well — see the header.
const wipeConfig = process.argv.includes('--all');
 ...
 *
 * Those same four `Restrict` relations are why the configuration group can be
 * kept: every row pointing INTO it is deleted above, so what is left is a
 * catalog and a location list with nothing hanging off them.
 */
async function clearAll(keepEmail: string): Promise<void> {
  const operational = [
    ...
    // Service delivery — values before their definitions
  ...
  await prisma.$transaction(
    wipeConfig ? [...operational, ...configuration] : operational,
  );
}
...
async function main(keepEmail: string): Promise<void> {
   ...
   */
  const scope = wipeConfig
    ? 'customer data and configuration cleared'
    : 'customer data cleared, catalog and settings kept';

  console.info(
    kept
      ? `Database reset — ${scope}; kept the admin account ${keepEmail}.`
      : `Database reset — ${scope}; no account exists for ${keepEmail}, run "npm run admin:setup" to create it.`,
  );
}
