import type { SupportFilter, SupportFilterOption } from '../../types/support';
 ...
 * source of truth for content (Design.md), so the strip is complete at every
 * width and scrolls horizontally rather than wrapping — which keeps the row
 * height predictable in a 300px pane. The scrollbar is hidden but the strip
 * still scrolls by touch, wheel, and keyboard.
 *
 * WHICH four is not this component's decision. A supervisor reads the whole
 * queue, so Unassigned/Assigned are cohorts they have; an agent's inbox is the
 * chats routed to them, so they get the workflow states instead. The backend
 * sends the right set with the list — the same reason the permission grid comes
 * from the API rather than being written out here.
 *
      ...
      {filters.map((filter) => {
        ...
        return (
          <button
            ...
            className={`shrink-0 whitespace-nowrap rounded-pill px-3.5 py-2 text-[0.8125rem] font-semibold transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary md:px-2.5 md:py-1.5 md:text-small lg:px-3.5 lg:py-2 ${
              isActive
