 ...
 *
 * The column widths sum to the table's declared minimum rather than overrunning
 * it. `table-fixed` scales every column down proportionally when the declared
 * widths exceed the table's width, so the previous allocation (68.75rem of
 * columns inside a 62.5rem minimum) left the action column ~10.8rem to hold a
 * 12.5rem pair of buttons — "Mark received" ran under the status chip beside it.
 */
      ...
      <table className="data-table min-w-[54rem] table-fixed lg:min-w-[64rem]">
        <thead>
          <tr className="h-12">
            ...
            <th scope="col" className="w-[8rem] pr-3 lg:pr-4">
              Amount
            </th>
            <th scope="col" className="hidden w-[10rem] pr-4 lg:table-cell">
              Send to
            </th>
            <th scope="col" className="w-[9.5rem] pr-3 lg:pr-4">
              Raised
            </th>
