/*
 * Documents pagination — two presentations of one cursor-paginated stream
 * (AGENTS.md), matching the rest of the portal:
 *   - desktop & tablet: "Page X of Y" with Previous / Next
 *   - mobile:           a "Load more" button over "Showing N of M"
 *
 * A disabled control renders in the muted style rather than disappearing, so the
 * layout stays stable.
 */
...
type DocumentsPaginationProps = {
  ...
  totalPages: number;
  totalItems: number;
  loadedCount: number; // documents on screen on mobile
  hasMore: boolean;
  onPrev?: () => void;
  onNext?: () => void;
  onLoadMore?: () => void;
    ...
    <>
      ...
      <div className="flex w-full flex-col items-center gap-2 pt-1 md:hidden">
        <button
          ...
          onClick={onLoadMore}
          disabled={!hasMore}
          className="flex h-10 items-center justify-center rounded-input border border-primary bg-white px-6 text-[0.8125rem] font-semibold text-primary transition-colors hover:bg-primary-light disabled:cursor-default disabled:border-gray-200 disabled:text-gray-400 disabled:hover:bg-white"
        >
          Load more
        </button>
